...
def joint_pos_limit_normalized(
    ...
    #joint_min = asset.data.soft_joint_pos_limits[:, joint_ids, 0]
    #joint_max = asset.data.soft_joint_pos_limits[:, joint_ids, 1]

    # Выводим первую среду (env 0)  
    #print("=== JOINT LIMITS FOR ENV 0 ===")
    #for i, name in enumerate(asset.joint_names):
    #    jpos = math.degrees(joint_pos[0, i].item())
    #    jmin = math.degrees(joint_min[0, i].item())
    #    jmax = math.degrees(joint_max[0, i].item())
    #    print(f"{i:2d}. {name:30s}  pos: {jpos:+.3f}  min: {jmin:+.3f}  max: {jmax:+.3f}")

    ###
    
    
    asset: Articulation = env.scene[asset_cfg.name]
    res_ = math_utils.scale_transform(
        asset.data.joint_pos[:, asset_cfg.joint_ids],
        asset.data.soft_joint_pos_limits[:, asset_cfg.joint_ids, 0],
        asset.data.soft_joint_pos_limits[:, asset_cfg.joint_ids, 1],
    )
    res_ = torch.clamp(res_, -1.0, 1.0)
    # print(res_)
...
###############################
#### ADDED FOR AnimalMath #####  
######## MIMIC TASK ###########     

    
def target_axis_cmd_norm(env: "MathManagerBasedRLEnv") -> torch.Tensor:
    # print("Miander:", env.miander_targets)
    return env.targets

def target_axis_swtchr_mask(env: "MathManagerBasedRLEnv") -> torch.Tensor:
    # print('mask', env.switcher_mask.float())
    return env.switcher_mask.float()
 
# MATH TELEOP    
def commands_udp_read(env: "MathManagerBasedRLEnv") -> torch.Tensor:
    # print('mask', env.switcher_mask.float())
    return env._ext_speed.float()   

def target_axis_cmd_norm_udp_read(env: "MathManagerBasedRLEnv") -> torch.Tensor:
    # print("Miander:", env.miander_targets)
    return env.targets.float()

def target_axis_swtchr_mask_udp_read(env: "MathManagerBasedRLEnv") -> torch.Tensor:
    # print('mask', env.switcher_mask.float())
    return env.switcher_mask.float()    
