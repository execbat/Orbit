...
from .math_teleop_manager_based_rl_env_cfg import MathTeleopManagerBasedRLEnvCfg
import socket


_PKT_LEN, _AXES = 49, 23
_DTYPE = np.float32
...
class MathTeleopManagerBasedRLEnv(MathManagerBasedEnv, gym.Env):
    ...
    def __init__(self, cfg: MathTeleopManagerBasedRLEnvCfg, render_mode: str | None = None, **kwargs):
        print('OLOLO')
        ...
        Args:
            ...
            render_mode: The render mode for the environment. Defaults to None, which
                is similar to ``"human"``.
        """
        self._dbg_every = 5  # печатать примерно раз в 120 шагов
       ...
       # joint init pos normalised
        ...
        0.736842155456543, 0.0625, 0.0625, 0.0, 0.0, -1.0, 1.0, -0.9512194991111755, -0.9512194991111755, 0.0, 0.0, \
        0.2857142686843872, 0.2857142686843872, 0.7333333492279053, 0.7333333492279053, 0.0, 0.0, 0.0, 0.0], 
            dtype=torch.float)
        
        # UDP connection server. list
        self.udp_addr: Tuple[str, int] = ("127.0.0.1", 55001)
        self._sock = create_socket(*self.udp_addr)
        ...
        # служебные буферы
        self.targets = torch.zeros(self.size[0], _AXES, device=self.dev)
        self.switcher_mask = torch.zeros_like(self.targets, device=self.dev)
        self._ext_speed = torch.zeros(self.size[0], 3, device=self.dev) 
        ...
        print("JOINT ORDER:", self.scene["robot"].data.joint_names)
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        """
        # читаем новый пакет (если есть)
        self._poll_udp()
        
        # put my commands from UDP
        cmd_term_vel = self.command_manager.get_term("base_velocity") 
        cmd_term_vel.command.copy_(self._ext_speed)    # (N_envs, 3):  vx, vy, ω_z   
        ...
        #!! Put our UDP commant into observation    
        ##########################################
        cmd_term_target_pos = self.command_manager.get_term("target_joint_pose") 
        cmd_term_target_pos.command.copy_(self.targets)    # (N_envs, 23):  axis target poses normalized 
        
        cmd_term_mask = self.command_manager.get_term("dof_mask") 
        cmd_term_mask.command.copy_(self.switcher_mask)    # (N_envs, 3):  axis mask 
        ##########################################   
        ...
        # return self.obs_buf, _, self.reset_terminated, self.reset_time_outs, self.extras
    ...
    def _reset_idx(self, env_ids: Sequence[int]):
        ...
        #new_lr = self._update_adaptive_curriculum(mean_tracking_reward)
    ...
    #    return (2 * torch.rand((len(env_ids), self.NUM_AXIS), device=self.dev) - 1.0) * self.MIANDER_SCALE
...
#            self.MASK_PROB_LEVEL = self.adaptive_state.get_mask_prob()
    ...
    #    n_envs, n_obs = self.size
    #    
    #    self.periods = torch.linspace(start=self.MIN_PERIOD, end=self.MIN_PERIOD, steps=n_envs, dtype=torch.float32, device = self.dev)
    ...
    #    """
    #    Build a single tensor `self.targets` of shape (n_envs, n_obs) that
    #    stores a sine wave for every environment.
    #
    #   targets[i, j] =  MIANDER_SCALE · sin(2π · j / period_i)

    #    where  
    #    • i — index of environment (row)  
    #    • j — index along the observation/axis dimension (column)
    #    """
    #    n_envs, n_obs = self.size                      # unpack once

        # (1) sample indices along the "axis" dimension: 0 … n_obs-1
    #    idx = torch.arange(n_obs, device=self.dev, dtype=torch.float32)   # shape (n_obs,)

        # (2) periods per-environment, (n_envs, 1) for broadcasting
    #    periods = self.periods.to(torch.float32).view(-1, 1)              # shape (n_envs,1)

        # Guard-rail: avoid division by zero in case a period accidentally == 0
    #    if torch.any(periods == 0):
    #        raise ValueError("Period must be non-zero for all environments.")

        # (3) sine wave with amplitude MIANDER_SCALE
    #    self.targets = self.MIANDER_SCALE * torch.sin(2 * math.pi * idx / periods)
        # self.targets: shape (n_envs, n_obs), dtype=float32, device=self.dev
        
        
    def _poll_udp(self) -> None:
        """Читаем все доступные пакеты и оставляем самый свежий."""
        last_msg = None
        while True:
            try:
                msg, _ = self._sock.recvfrom(_PKT_LEN * 4)
                last_msg = msg
            except BlockingIOError:
                break                 # очередь пуста

        if last_msg is None:         # пакетов не было
            return

        vec = np.frombuffer(last_msg, dtype=_DTYPE)
        if vec.size != _PKT_LEN:
            print("incorrect size of incoming telegram")
            return
            
        #print(vec)    

        tgt, msk, spd = vec[:_AXES], vec[_AXES:2*_AXES], vec[-3:]
        tgt_t = torch.as_tensor(tgt, device=self.dev).view(1, -1).expand(self.num_envs, -1)
        msk_t = torch.as_tensor(msk, device=self.dev).view(1, -1).expand(self.num_envs, -1)
        spd_t = torch.as_tensor(spd, device=self.dev).view(1, -1).expand(self.num_envs, -1)

        self.targets.copy_(tgt_t)
        self.switcher_mask.copy_(msk_t)
        self._ext_speed.copy_(spd_t)
        #print(f"targets {self.targets}")
        #print(f"switcher_mask {self.switcher_mask}")
        #print(f"_ext_speed {self._ext_speed}")        
        #print()
